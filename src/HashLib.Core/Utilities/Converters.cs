...
namespace HashLib.Core.Utilities
{
    public static class Converters
    {
        public static byte[] ConvertToBytes(object a_in)
        {
            if (a_in is null)
            {
                throw new ArgumentNullException(nameof(a_in));
            }

            return a_in switch
            {
                byte a_out => new byte[] { a_out },
                short a_out => BitConverter.GetBytes(a_out),
                ushort a_out => BitConverter.GetBytes(a_out),
                char a_out => BitConverter.GetBytes(a_out),
                int a_out => BitConverter.GetBytes(a_out),
                uint a_out => BitConverter.GetBytes(a_out),
                long a_out => BitConverter.GetBytes(a_out),
                ulong a_out => BitConverter.GetBytes(a_out),
                float a_out => BitConverter.GetBytes(a_out),
                double a_out => BitConverter.GetBytes(a_out),
                string a_out => ConvertStringToBytes(a_out),
                byte[] a_out => (byte[])a_out.Clone(),
                var a_out when a_out.GetType().IsArray && a_out.GetType().GetElementType() == typeof(short) => ConvertShortsToBytes((short[])a_out),
                var a_out when a_out.GetType().IsArray && a_out.GetType().GetElementType() == typeof(ushort) => ConvertUShortsToBytes((ushort[])a_out),
                char[] a_out => ConvertCharsToBytes(a_out),
                var a_out when a_out.GetType().IsArray && a_out.GetType().GetElementType() == typeof(int) => ConvertIntsToBytes((int[])a_out),
                var a_out when a_out.GetType().IsArray && a_out.GetType().GetElementType() == typeof(uint) => ConvertUIntsToBytes((uint[])a_out),
                var a_out when a_out.GetType().IsArray && a_out.GetType().GetElementType() == typeof(long) => ConvertLongsToBytes((long[])a_out),
                var a_out when a_out.GetType().IsArray && a_out.GetType().GetElementType() == typeof(ulong) => ConvertULongsToBytes((ulong[])a_out),
                float[] a_out => ConvertFloatsToBytes(a_out),
                double[] a_out => ConvertDoublesToBytes(a_out),
                _ => throw new ArgumentException("Unable to convert", nameof(a_in))
            };
        }
